...
class User:
    ...
    Attributes:
        ...
        password (Union[Unset, str]): If the password has no known hashing algo prefix it will be stored, by default,
            using bcrypt, argon2id is supported too. You can send a password hashed as bcrypt ($2a$ prefix), argon2id,
            pbkdf2 or unix crypt and it will be stored as is. For security reasons this field is omitted when you search/get
            users
        public_keys (Union[Unset, list[str]]): Public keys in OpenSSH format.
        has_password (Union[Unset, bool]): Indicates whether the password is set
